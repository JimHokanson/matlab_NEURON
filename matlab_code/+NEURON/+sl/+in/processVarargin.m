...
%
%   [in,extras] = NEURON.sl.in.processVarargin(in,v,varargin) 
%
%   Inputs:
%   -------
...
%   Outputs:
%   --------
%   extras: NEURON.sl.in.process_varargin_result
%
%   Examples:
%   ---------
%   1)
...
%   in.a = 1
%   in.b = 2
%   in  NEURON.sl.in.processVarargin(in,varargin,'allow_duplicates',true)
%
%   Similar functions:
%   ------------------
...
%
%   See Also:
%   NEURON.sl.in.tests.processVarargin
...
if ~isempty(varargin)
    ...
    %c  NEURON.sl.in.processVararginHelper(c,varargin,c2,1);
    c = NEURON.sl.in.processVararginHelper(c,varargin,c,true);
end
...
if c.return_as_object
   in = NEURON.sl.in.optional_inputs(in); 
end
...
%
%   [in,extras]  NEURON.sl.in.processVararginHelper(in,v,c,is_parsing_options)
%
...
extras = NEURON.sl.in.process_varargin_result(in,v);
...
if ~all(is_present)
    if c.allow_non_matches
        ...
        error(['Bad variable names given in input structure: ' ...
            '\n--------------------------------- \n %s' ...
            ' \n--------------------------------------'],...
            NEURON.sl.cellstr.join(badVariables,'d',','))
    end
end
