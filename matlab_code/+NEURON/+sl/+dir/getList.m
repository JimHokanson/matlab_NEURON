...
%   an object
%   
%   list_result = NEURON.sl.dir.getList(root_folder_path,varargin)
%
%
%   2) Only return certain info, not the object
%   
%   file_names = NEURON.sl.dir.getList(root_folder_path,...
%                       'output_type','names',varargin)
%
%
%   3) Return folders instead of files & returns as full paths
%   
%   folder_paths = NEURON.sl.dir.getList(root_folder_path,'output_type','paths','search_type',folders',varargin)
%
%
%   4) File and folders when 'output_type' is not 'list_result'
%   
%   [file_names,folder_names] = NEURON.sl.dir.getList(root_folder_path,...
...
%   ----------------------------------
%   list_result : NEURON.sl.dir.list_result
...
%       - object : return NEURON.sl.dir.list_result
...
%   list_result = NEURON.sl.dir.getList(folder_path,'extension','.mat')
%
%   2) Find files of a given type in a directory AND subdirectories
%   list_result = NEURON.sl.dir.getList(root_path,'recursive',true,'extension','.mat')
%
%   3) Find all nested folders starting with the word 'block'
%   list_result = NEURON.sl.dir.getList(root_path,'recursive',true,...
%       'search_type','folders','folder_pattern','block*')
%
%   4) Recursive file search where only full file paths are needed
%   file_paths = NEURON.sl.dir.getList(root_path,'recursive',true,'output_type','paths')
%
%
%   Still to do:
%   ------------
...
%{
    ...
    wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','search_type','folders','first_chars_in_folders_to_ignore','+@')

    %Non-recursive with an extension
    wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','extension','.m')

    %Non-recursive list of files and folders
    wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','search_type','both')

    %Different return type ...
    [file_names,folder_names] = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN',...
            'search_type','both','output_type','names')

    %Recursive
	wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','recursive',true)

    %Recursive, only folders
	wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','recursive',true,'search_type','folders')

    %Recursive with an extension
	wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','recursive',true,'extension','.m')

    %Recursive with a pattern
    wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','recursive',true,'file_pattern','g*.m')

    %Recursive with an extension - but needing dir info
    wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','recursive',true,'extension','.m','need_dir_props',true)

    %Recursive with an extension and size filter
    %Keep files that are less than 6000 bytes
    wtf = NEURON.sl.dir.getList('C:\D\repos\matlab_git\matlab_SVN','recursive',true,'file_pattern','*.m','file_size_filter',@(x)(x < 6000))


    tic;
	wtf = NEURON.sl.dir.rdir('C:\D\repos\matlab_git\matlab_SVN\**\*.m');
    toc;
...
end  %------------------------  End of NEURON.sl.dir.getList ---------------------
...
s.folder_names = NEURON.sl.dir.getFileName(s.folder_paths);
...
file_names = NEURON.sl.dir.getFileName(file_paths);
...
s.file_names   = file_names;
s.folder_names = {};
s.file_paths   = NEURON.sl.dir.fullfileCA(root_folder_path,file_names);
s.folder_paths = {};
s.d_files      = d;
s.d_folders    = [];
...
s.file_paths   = {};
s.folder_paths = NEURON.sl.dir.fullfileCA(root_folder_path,folder_names);
s.d_files      = [];
s.d_folders    = d;
