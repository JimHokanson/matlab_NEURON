...
%   $Revision: 5.34.4.14 $  $Date: 2011/05/17 02:32:28 $
  ...
  elseif ( ischar(varargin{end-1}) && isnumeric(varargin{end}) )
    ...
    if method(1)=='*', % Direct call bypass.
        ...
        elseif method(2)=='s', % Spline
            ...
            error(message('MATLAB:interp3:InvalidMethod', deblank( method )));
...
if (size(x,2)>1 && ~isequal(repmat(xx, [size(x,1) 1 size(x,3)]),x)) || ...
   ...
   (size(z,3)>1 && ~isequal(repmat(zz, [size(z,1) size(z,2) 1]),z)),
    error(message('MATLAB:interp3:meshgrid'));
end
...
if ~bypass,
  ...
  if (xdiff > eps*max(xx)) || (ydiff > eps*max(yy)) || (zdiff > eps*max(zz))
    ...
    if any(dx<=0) || any(dy<=0) || any(dz<=0), 
      error(message('MATLAB:interp3:XYorZNotMonotonic'));
    end
...
if nargin==2, % linear(v), Expand V
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:linear:Vnot3Dnarg2')); 
  end
...
elseif nargin==4 || nargin==6 || nargin==7
  error(message('MATLAB:interp3:linear:ninput'));

elseif nargin==5, % linear(v,s,t,w), No X, Y or Z specified.
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:linear:Vnot3Dnarg5')); 
  end
  [nrows,ncols,npages] = size(arg1);
...
elseif nargin==8, % linear(x,y,z,v,s,t,w), X, Y and Z specified.
  if ndims(arg4)~=3 
    error(message('MATLAB:interp3:linear:Vnot3Dnarg8')); 
  end
...
if any([nrows ncols npages]<[2 2 2]), 
  error(message('MATLAB:interp3:linear:Vsize'));
end
if ~isequal(size(s),size(t),size(w))
  error(message('MATLAB:interp3:linear:XIYIZISizeMismatch'));
end
...
if nargin==2, % cubic(v), Expand V
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:cubic:Vnot3D')); 
  end
  [nrows,ncols,npages] = size(arg1); 
  [s,t,w] = meshgrid(1:.5:ncols,1:.5:nrows,1:.5:npages);
...
elseif nargin==3, % cubic(v,n), Expand V n times
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:cubic:Vnot3Dnarg3')); 
  end
  [nrows,ncols,npages] = size(arg1);
  ntimes = floor(arg2); delta = 1/(2^ntimes);
  [s,t,w] = meshgrid(1:delta:ncols,1:delta:nrows,1:delta:npages);
...
elseif nargin==4 || nargin==6 || nargin==7
  error(message('MATLAB:interp3:cubic:WrongInputNum'));

elseif nargin==5, % cubic(v,s,t,w), No X, Y or Z specified.
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:cubic:Vnot3Dnarg5')); 
  end
  [nrows,ncols,npages] = size(arg1);
  s = arg2; t = arg3; w = arg4;
...
elseif nargin==8, % cubic(x,y,z,v,s,t,w), X, Y and Z specified.
  if ndims(arg4)~=3 
    error(message('MATLAB:interp3:cubic:Vnot3Dnarg8')); 
  end
  [nrows,ncols,npages] = size(arg4);
  mx = numel(arg1); my = numel(arg2); mz = numel(arg3);
  if ~isequal([my mx mz],size(arg4)) && ...
     ~isequal(size(arg1),size(arg2),size(arg3),size(arg4)),
    error(message('MATLAB:interp3:cubic:XYZLengthMismatchV'));
  end
  s = 1 + (arg5-arg1(1))/(arg1(mx)-arg1(1))*(ncols-1);
  t = 1 + (arg6-arg2(1))/(arg2(my)-arg2(1))*(nrows-1);
  w = 1 + (arg7-arg3(1))/(arg3(mz)-arg3(1))*(npages-1);
...
if any([nrows ncols npages]<[3 3 3]),
  error(message('MATLAB:interp3:cubic:Vsize'));
end
if ~isequal(size(s),size(t),size(w)),
  error(message('MATLAB:interp3:cubic:XIYIZISizeMismatch'));
end
...
if nargin==2, % nearest(v), Expand V
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:nearest:Vnot3D')); 
  end
  [nrows,ncols,npages] = size(arg1); 
  [s,t,w] = meshgrid(1:.5:ncols,1:.5:nrows,1:.5:npages);
...
elseif nargin==3, % nearest(v,n), Expand V n times
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:nearest:Vnot3Dnarg3')); 
  end
  [nrows,ncols,npages] = size(arg1);
  ntimes = floor(arg2); delta = 1/(2^ntimes);
  [s,t,w] = meshgrid(1:delta:ncols,1:delta:nrows,1:delta:npages);
...
elseif nargin==4 || nargin==6 || nargin==7
  error(message('MATLAB:interp3:nearest:WrongInputNum'));

elseif nargin==5, % nearest(v,s,t,w), No X, Y or Z specified.
  if ndims(arg1)~=3 
    error(message('MATLAB:interp3:nearest:Vnot3Dnarg5')); 
  end
  [nrows,ncols,npages] = size(arg1);
  s = arg2; t = arg3; w = arg4;
...
elseif nargin==8, % nearest(x,y,z,v,s,t,w), X, Y and Z specified.
  if ndims(arg4)~=3 
    error(message('MATLAB:interp3:nearest:Vnot3Dnarg8')); 
  end
  [nrows,ncols,npages] = size(arg4);
  mx = numel(arg1); my = numel(arg2); mz = numel(arg3);
  if ~isequal([my mx mz],size(arg4)) && ...
     ~isequal(size(arg1),size(arg2),size(arg3),size(arg4)),
    error(message('MATLAB:interp3:nearest:XYZLengthMismatchV'));
  end
...
if ~isequal(size(s),size(t),size(w)),
  error(message('MATLAB:interp3:nearest:XIYIZISizeMismatch'));
end
